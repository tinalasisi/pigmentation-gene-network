...
| NB4 (reframed 2026-07-12): effector-status re-read of 105 curated loci → 14 effector-uncertain (target) vs 75 canonical-variant-gap; 27 tagged loci rsID-overlap GWAS Catalog (cross-check); unified base 1,177 rows | Notebook's own TL;DR + printed cells + `discordance_loci_effector_classified.csv` | **Internally consistent** (105 + 1,072 = 1,177; effector counts verified against the committed CSV). The earlier coarse "52 author-unexplained" framing is superseded (it overstated the open set ~6×). Kim 2024 is now folded into the effector re-classification (was a documented gap). |
