...
- `nb4_unified_association_base.csv`, `discordance_loci_effector_classified.csv` — NB4 unified base + per-locus effector-status classification (the 105 legacy rows, `paper != "Kim2024"`, are NB4's curated input). The superseded `discordance_loci_author_explained.csv` was retired to `internal/archive/superseded_2026-07-12/` on 2026-07-12 (its two audit columns migrated into the effector-classified file first).
