...
| 14 | **Maintain handoff docs in `internal/handoffs/`.** New platform-to-platform handoff documents follow the `HANDOFF_<FROM-PLATFORM>_TO_<TO-PLATFORM>_<topic>.md` convention in `internal/handoffs/README.md`; agent-to-agent operational notes go to `internal/handoffs/notes/` (one file per message, see `MERGE_SAFETY.md`), never as free-text edits to `CHANGELOG.md`. See `CHANGELOG.md` 2026-07-11 entry (handoffs reorganization) and `project_dashboard.md` §6. | governance | PM / any agent writing a handoff or note | No — ongoing hygiene, not build-blocking |
| 15 | **AGREED (2026-07-11, two-level framing adoption).** Extend the dark-matter ledger to all 15 genes with a cited resolution class (positional-pointer / redirect-to-other-gene / LD-passenger-or-no-signal / genuinely-novel — see `RESEARCH_SYNTHESIS_locus_resolution_mvp.md` §1 and `FRAMING_EVALUATION_dark_matter.md`). | dark-matter second act | build | No — active work item, not blocking the current deliverable |
...
| D8 | **HELD until after the MVP (PI decision, 2026-07-11 — not open, not scheduled).** Widen the corpus to GWAS Catalog ≥2×-replicated associations as `dark_matter_association` loci. The PI reviewed this expansion alongside the two-level framing adoption and explicitly decided to hold it, not to authorize or reject it outright — record as held, do not treat as open for discussion at the next checkpoint until the MVP lands. | `FRAMING_EVALUATION_dark_matter.md` §5/§6; `CHANGELOG.md` 2026-07-11 framing-adoption entry |
