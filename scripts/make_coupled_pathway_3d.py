...
INK = "#26303a"
AXC = {"melanocortin": "#2b8a7a", "estrogen": "#c85a1b", "androgen": "#7b4ea3", "endothelin": "#2c7fb8",
       "core": "#3f4a52", "pigment": "#2f9e8f", "product": "#2a2a2a"}
...
N = {
 "aMSH": (*corner("melanocortin", 3.4, 3.55), "α-MSH", "ligand", "melanocortin", "Melanocortin ligand (cleaved from POMC)"),
 "E2":   (*corner("estrogen", 3.4, 3.55), "Estradiol", "ligand", "estrogen", "17β-estradiol (endocrine + local aromatase)"),
 "DHT":  (*corner("androgen", 3.4, 3.55), "DHT", "ligand", "androgen", "Dihydrotestosterone (5α-reduced androgen)"),
 "EDN3": (*corner("endothelin", 3.4, 3.55), "EDN3", "ligand", "endothelin", "Endothelin-3 (keratinocyte-derived)"),
 ...
 "cAMP":  (-1.0, 0.25, 2.0, "cAMP", "core", "core", "Shared 2nd messenger — melanocortin AND estrogen(GPER) converge here. Filardo 2002. PMID 11773440"),
 "MITF":  (0.0, 0.0, 1.05, "MITF", "core", "core", "Master melanocyte transcription factor — the waist of the hourglass: signals converge here, melanogenesis begins here"),
 "SOX10": (1.2, 1.2, 1.55, "SOX10", "pigment", "pigment", "Transcription factor — activates MITF and co-activates the melanin enzymes (TYR, TYRP1, DCT)"),
 "MEL":   (0.0, 0.0, -1.0, "Melanin", "product", "product", "Melanin — the pigment produced (eumelanin / pheomelanin)"),
}
...
for k, (g, desc) in enumerate(MEL):
    ...
    N[g] = (1.9*math.cos(ang), 1.9*math.sin(ang), 0.2, g, "pigment", "pigment", desc)
...
for s, d, ax, est in E:
    ...
    fig.add_trace(go.Scatter3d(x=[x0, x1], y=[y0, y1], z=[z0, z1], mode="lines",
        line=dict(color=AXC[ax], width=6 if est else 3), opacity=0.95 if est else 0.5,
        hoverinfo="skip", showlegend=False))
KS = {"ligand": (17, 0.22), "recept": (23, 1.0), "core": (28, 1.0), "pigment": (21, 1.0), "product": (40, 0.95)}
for kind in ("ligand", "recept", "core", "pigment", "product"):
    ids = [i for i in N if N[i][4] == kind]
    fig.add_trace(go.Scatter3d(x=[N[i][0] for i in ids], y=[N[i][1] for i in ids], z=[N[i][2] for i in ids],
        mode="markers+text", text=[N[i][3] for i in ids], textposition="middle center",
        textfont=dict(size=12.5, color="white" if kind in ("recept", "core", "pigment", "product") else INK),
        marker=dict(size=KS[kind][0], color=[AXC[N[i][5]] for i in ids], opacity=KS[kind][1],
                    line=dict(color="#33404a", width=1)), customdata=ids,
        hoverinfo="text", hovertext=[f"<b>{N[i][3]}</b><br>{N[i][6]}" for i in ids], showlegend=False))
for z, lab in [(3.55, "Diffusible signal"), (3.0, "Melanocyte receptor"), (2.0, "2nd messenger"),
               (1.05, "MITF · convergence"), (0.2, "Melanogenesis"), (-1.0, "Melanin")]:
    fig.add_trace(go.Scatter3d(x=[-4.6], y=[0], z=[z], mode="text", text=[lab],
        textfont=dict(size=10, color="#8a949b"), hoverinfo="skip", showlegend=False))

fig.update_layout(
    title=dict(text="<b>From signal to pigment — the hourglass, in 3D</b><br>"
        "<span style='font-size:13px;color:#5c656b'>Four hormone axes funnel down onto MITF (the waist); MITF + SOX10 then fan out to the "
        "melanogenesis genes → melanin. Thick = established · thin = weak/regional. Drag to rotate · click a node.</span>",
    ...
    paper_bgcolor="white", height=760, margin=dict(l=0, r=0, t=92, b=0), showlegend=False)

# ---- dossier ----
KINDLBL = {"ligand": "Diffusible signal (ligand)", "recept": "Melanocyte receptor", "core": "Pathway core",
           "pigment": "Melanogenesis gene", "product": "Pigment product"}
...
INFO = {i: {"label": N[i][3].replace("\n", " "), "kind": KINDLBL[N[i][4]], "axis": AXLBL[N[i][5]],
            "desc": N[i][6], "acol": AXC[N[i][5]], "down": down[i], "up": up[i]} for i in N}
...
function showNode(id){var d=INFO[id];if(!d)return;
 ...
 document.getElementById('d-badges').innerHTML='<span class="badge" style="background:'+d.acol+'">'+d.axis+'</span><span class="badge" style="background:#5c656b">'+d.kind+'</span>';
